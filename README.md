...
- Gate 3 WebDAV 验收填写模板：`docs/plans/2026-03-01-gate3-webdav-critical-flow-template.md`
- 验收辅助脚本：
...
- 移动端 UI：
  ...
  - 底部导航增加当前区块高亮与平滑锚点滚动，并修复锚点跳转后高亮偶发回跳，减少长页面定位成本。
  ...
  - 移动端页面补充 `viewport-fit=cover`，与 `safe-area` 内边距策略保持一致，减少刘海屏/底部手势区贴边风险。
  - 日志区域补充“可上下滑动查看更多”提示，并强化可滚动区域边界与触控滚动体验。
  - 日志/版本/活动列表自动刷新时尽量保留当前阅读滚动位置；用户实际滚动后自动收起“可滑动提示”，减少阅读打断。
...
- 可访问性基线：
  ...
  - 浅色主题次级文案颜色对比度提升（`--muted`），改善小字号说明文字可读性。
