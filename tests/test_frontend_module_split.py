...
class FrontendModuleSplitTests(unittest.TestCase):
    def test_index_loads_api_and_formatter_modules_before_app(self) -> None:
        ...
        self.assertLess(bootstrap_pos, app_runtime_pos)
        self.assertLess(bootstrap_pos, app_pos)
    ...
    def test_app_bootstrap_uses_shared_global_modules(self) -> None:
        ...
        self.assertIn("window.GRWBootstrapContract", app_js)
        self.assertIn("bootstrap.requireModules(", app_js)
        self.assertIn('"GRWApiClient"', app_js)
        self.assertIn('"GRWFormatters"', app_js)
        self.assertIn('"GRWLogsView"', app_js)
        self.assertIn('"GRWRepoController"', app_js)
        self.assertIn('"GRWSettingsController"', app_js)
        self.assertIn('"GRWStorageDiagnostics"', app_js)
        self.assertIn('"GRWBatchSelectors"', app_js)
        self.assertIn('"GRWBatchActions"', app_js)
        self.assertIn('"GRWMobileBehavior"', app_js)
        self.assertIn('"GRWAppRuntime"', app_js)
